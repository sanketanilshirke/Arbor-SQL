...
-- Question 0: Retrieve a list of hotels along with their average rating from reviews.
select h.hotelid, h.hotelname, avg(r.rating) as avg_rating
from hotels h
left join hotelreviews r on h.hotelid = r.hotelid
group by h.hotelid, h.hotelname;

select h.hotelid, h.hotelname, round(avg(r.rating), 2) as avg_rating
from hotels h
left join hotelreviews r on h.hotelid = r.hotelid
group by h.hotelid, h.hotelname;
