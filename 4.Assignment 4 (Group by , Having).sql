...
select category, sum(sale_amount) as TOTAL from sales group by category;
...
select category, max(sale_amount) as TOTAL from sales group by category;

-- 4-	Which categories have an average sale amount below 50?              
select category, avg(sale_amount) as avg_sale from sales group by category having avg(sale_amount)<50;
...
select product_name, count(*) from sales group by product_name having count(*)>5;

-- 7-	What is the average sale amount for products with equal to or more than 2 sales?
select product_name, avg(sale_amount) from sales group by product_name having count(*)>=2;
...
select product_name, sum(sale_amount) from sales group by product_name order by sale_amount desc limit 1;
...
select year(sale_date) ,month(sale_date), avg(sale_amount) from sales group by year(sale_date), month(sale_date);

-- 12-	What are the products with the highest sale amount in each category?
select category, product_name, sale_amount from sales where (category, sale_amount) in ( select category, max(sale_amount) from sales group by category);
...
-- 14-	In which months are there more than 3 sales?
select year(sale_date) as sale_year,month(sale_date) as sale_month,count(*) as total_sales from sales
group by year(sale_date), month(sale_date)
having count(*) > 3;


-- 15-	What is the average sale amount for each product category in months with more than 3 sales?
select category, year(sale_date) as sale_year, month(sale_date) as sale_month, avg(sale_amount) as avg_sale from sales
