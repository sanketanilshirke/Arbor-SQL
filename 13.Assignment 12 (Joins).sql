...
select concat_ws(' ',e.firstname,e.lastname) as fullname, d.departmentname from employee as e join department as d 
...
select d.departmentname, max(e.salary) from employee e join department d
on e.departmentid = d.departmentid group by d.departmentname;
...
select d.departmentname, count(e.employeeid) from employee e join department d
on e.departmentid = d.departmentid group by d.departmentname order by count(e.employeeid) desc limit 1;  

select d.departmentname, count(e.employeeid) from employee e join department d
on e.departmentid = d.departmentid group by d.departmentname having count(e.employeeid)=
...
select e.EmployeeID, e.firstname,e.lastname from employee e left join department d
...
select  e.firstname,e.lastname, d.departmentname from employee e join department d
on e.departmentid = d.departmentid where 
e.departmentid = (select departmentid from employee where firstname='amit' and lastname='sharma');

select EmployeeID, FirstName, LastName, Email, Salary, DateOfJoining, DepartmentName 
from employee as e
join department as d
on e.Departmentid = d.Departmentid
where DepartmentName = (select DepartmentName from department as d
join employee as e
on e.Departmentid = d.Departmentid
